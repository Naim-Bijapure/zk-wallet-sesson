...
contract SocialRecovery is Ownable {
  ...
  function setup(bytes32[] memory guardianAddrHashes, uint256 _threshold) external beforeSetup onlyOwner {
  ...
  function initRecovery(address _proposedOwner) external afterSetup onlyGuardian notInRecovery {
  ...
  function supportRecovery(address _proposedOwner) external afterSetup onlyGuardian onlyInRecovery {
    guardianToRecovery[msg.sender] = Recovery(_proposedOwner, currRecoveryRound, false);
    emit RecoverySupported(msg.sender, _proposedOwner, currRecoveryRound);
  }

  function cancelRecovery() external afterSetup onlyOwner onlyInRecovery {
    inRecovery = false;
  ...
  ) external afterSetup onlyGuardian onlyInRecovery {
  ...
  function transferGuardianship(bytes32 newGuardianHash) external afterSetup onlyGuardian notInRecovery {
    require(
  ...
  function initiateGuardianAddition(bytes32 guardianHash) external afterSetup onlyOwner {
  ...
  function executeGuardianAddition(bytes32 newGuardianHash) external afterSetup onlyOwner {
  ...
  function cancelGuardianAddition(bytes32 guardianHash) external afterSetup onlyOwner {
    guardianHashToAdditionTimestamp[guardianHash] = 0;
  }

  function initiateGuardianRemoval(bytes32 guardianHash) external afterSetup onlyOwner {
  ...
  function executeGuardianRemoval(bytes32 oldGuardianHash) external afterSetup onlyOwner {
  ...
  function cancelGuardianRemoval(bytes32 guardianHash) external afterSetup onlyOwner {
    guardianHashToRemovalTimestamp[guardianHash] = 0;
  }

  function revealGuardianIdentity() external afterSetup onlyGuardian {
